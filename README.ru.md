```markdown
# Game-Tracker 🎮

[Read in English](README.md)

**Game-Tracker** — это бэкенд-сервис на .NET 10, созданный для удобного мониторинга игрового времени и фиксации этапов прохождения игр (поинтов).

Проект реализован с использованием **Clean Architecture** и паттерна **CQRS**, что делает код чистым, поддерживаемым и готовым к масштабированию.

---

## 🏗 Архитектура и Стек

* **Платформа:** .NET 8 / ASP.NET Core
* **Паттерны:** CQRS (MediatR), Clean Architecture
* **Валидация:** FluentValidation (интегрирована в Pipeline MediatR)
* **База данных:** PostgreSQL + Entity Framework Core
* **Инфраструктура:** Docker & Docker Compose
* **Обработка ошибок:** Кастомный Middleware для глобального отлова исключений

---

## 🛠 Реализованный функционал

- **CRUD для Игр:** Полное управление списком игр.
- **Трекинг времени:** Возможность записывать сессии и фиксировать время прохождения определенных этапов.
- **Global Exception Handling:** Middleware автоматически ловит ошибки валидации и возвращает понятные ответы.
- **Pipeline Behaviors:** Автоматическая проверка данных перед выполнением бизнес-логики.

---

## 📈 План развития

🛠 Реализованные возможности

    [x] Сервис аутентификации: Система регистрации и входа (Identity + JWT).

    [x] Инфраструктура: Полная докеризация проекта (API + PostgreSQL).

    [ ] Интеграция с Redis: Планируется для высокоскоростного кэширования.

🚀 Быстрый старт

    Клонируйте репозиторий:
    Bash

    git clone https://github.com/your-username/game-tracker.git

    Настройте окружение:
    Создайте файл .env в корневой директории (используйте .env.example как образец).

    Запустите проект через Docker:
    Bash

    docker-compose up -d --build

    API будет доступно по адресу: http://localhost:8080/swagger

Содержимое .env.example (для справки):

DB_USER=имя_пользователя
DB_PASSWORD=пароль
DB_NAME=имя_базы_данных
DB_PORT=5432
JWT_KEY=ваш_секретный_ключ_минимум_32_символа
JWT_ISSUER=TrackerAPI
JWT_AUDIENCE=TrackerUsers

---


Статус разработки

Проект находится в стадии активного наполнения функционалом. В ближайшее время ожидается добавление Redis.
